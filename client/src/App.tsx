...
import SimplePaymentModal from './components/Common/SimplePaymentModal';
import { PaymentHandler, PAYMENT_CONFIGS } from './components/Common/PaymentHandler';
...
const AppContent: React.FC = () => {
  ...
  const [inputModalConfig, setInputModalConfig] = useState<InputModalConfig>({
    title: '',
    placeholder: '',
    inputType: 'text',
    onConfirm: () => {}
  ...
  const [simplePaymentModal, setSimplePaymentModal] = useState({ isOpen: false, paymentData: null, onConfirm: null });
  ...
  const openSimplePaymentModal = useCallback((paymentData, onConfirm) => {
    setSimplePaymentModal({ isOpen: true, paymentData, onConfirm });
    window.Telegram?.WebApp?.HapticFeedback.impactOccurred('medium');
  }, []);
  ...
  const handleUpgradeItem = useCallback(
    (item) => {
      ...
      // Special handling for Auto Robot - one-time purchase only
      if (item.type === 'robot' && item.name === 'Auto Robot') {
        if (data.user?.hasAutoRobot) {
          notifier.addNotification("Auto Robot allaqachon sotib olingan!", 'error', 2000, 'robot-owned');
          return;
        }

        const requiredCoins = 1000000;
        if (data.user?.dubaiCoin < requiredCoins) {
          notifier.addNotification("1,000,000 coin kerak!", 'error', 2000, 'robot-error');
          return;
        }
        ...
        notifier.addNotification('Auto Robot sotib olindi!', 'success', 2000, 'robot-success');
        window.Telegram?.WebApp?.HapticFeedback.notificationOccurred('success');
        return;
      }
  ...
  return (
    ...
    >
      ...
      />
      <SimplePaymentModal
        isOpen={simplePaymentModal.isOpen}
        onClose={() => setSimplePaymentModal({ isOpen: false, paymentData: null, onConfirm: null })}
        paymentData={simplePaymentModal.paymentData}
        onConfirm={simplePaymentModal.onConfirm}
      />
    </div>
  );
};
