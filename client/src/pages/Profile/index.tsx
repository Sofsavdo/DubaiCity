...
const Profile = ({ user, setUser, openInputModal, openSimplePaymentModal, handleInviteFriend, levelNames, companies, exchangeRate, notifier }) => {
    ...
    const handleBotActivation = () => {
        // DubaiCity aktivatsiyasi - airdropga tayyorgarlik
        openSimplePaymentModal({
            name: 'DubaiCity Aktivatsiyasi',
            usdPrice: 0.50,
            description: 'Airdropga tayyorgarlik uchun aktivatsiya'
        }, () => {
            setUser(prev => ({
                ...prev,
                isActive: true,
                walletAddress: prev.walletAddress || 'auto_generated_wallet'
            }));
            if (notifier) {
                notifier.addNotification('DubaiCity aktivatsiya qilindi! 🚀', 'success', 3000);
            }
        });
    };

    const handlePremiumPurchase = () => {
        // Premium sotib olish - 2$ narx
        openSimplePaymentModal({
            name: 'Premium Status',
            usdPrice: 2.00,
            description: 'Premium status +50% daromad bonus va maxsus imkoniyatlar beradi'
        }, () => {
            setUser(prev => ({
                ...prev,
                isPremium: true
    ...
    const handleSponsorship = () => {
        // Homiylik qilish - o'zgaruvchan summa
        openInputModal({
            title: 'Homiylik miqdori',
            ...
            onConfirm: (amount) => {
                ...
                if (isNaN(donationAmount) || donationAmount <= 0) {
                    if (notifier) {
                        notifier.addNotification('Noto\'g\'ri summa kiritildi', 'error', 2000);
                    }
                    return;
                }
                openSimplePaymentModal({
                    ...
                    description: `${donationAmount}$ homiylik qilish`
                }, () => {
                    setUser(prev => ({
                        ...prev,
                    ...
                    if (notifier) {
                        notifier.addNotification('Homiylik uchun rahmat! 💝', 'success', 3000);
                    }
                });
            }
        });
             ...
             <div className="flex-grow overflow-y-auto px-3 pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                 <div className="glass-card p-4 space-y-3">
                              ...
                              <div className="space-y-1">
                                  <div className="flex items-center gap-2">
                                      ...
                                      <span className={user.walletAddress ? 'text-green-400' : 'text-red-400'}>
                                          DubaiCity aktivatsiyasi (0.5$ to'lov)
                                      </span>
                                  </div>
                                  <div className="flex items-center gap-2">
                      ...
                      <div className="space-y-3">
                          <h3 className="text-sm font-bold mb-2">To'lov Xizmatlari</h3>
                          <p className="text-xs text-gray-400 mb-2">🎯 Listing jarayoniga tayyorgarlik</p>
                          
                          {/* Bot Activation */}
                          {!user.isActive && (
                              <button 
                                  onClick={handleBotActivation}
                                  className="w-full bg-green-600 hover:bg-green-700 transition-colors text-white font-bold py-2 rounded-xl text-sm flex items-center justify-center gap-2"
                              >
                                  🚀 DubaiCity Aktivatsiya ($0.50)
                              </button>
                          )}
                          ...
                          {!user.isPremium && (
                              <button 
                                  ...
                                  className="w-full bg-purple-600 hover:bg-purple-700 transition-colors text-white font-bold py-2 rounded-xl text-sm flex items-center justify-center gap-2"
                              >
                                  👑 Premium Sotib Olish ($2.00)
                              </button>
                          )}
                          ...
                          <button 
                              ...
                              className="w-full bg-orange-600 hover:bg-orange-700 transition-colors text-white font-bold py-2 rounded-xl text-sm flex items-center justify-center gap-2"
                          >
                              💝 Homiylik Qilish (Ixtiyoriy)
                          </button>
                      </div>
                      ...
                      <div>
                          ...
                          {user.walletAddress ? (
                              ...
                              <div>
                                  <p className="text-xs text-gray-400 mb-2">Airdrop va kelajakdagi tokenlar uchun hamyoningizni ulang.</p>
