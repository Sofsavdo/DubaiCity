...
const Company = ({ user, companies, handleJoinCompany, handleCreateCompany, handleLeaveCompany, handleSendMessage, openPaymentModal, openInputModal, levelNames, handleDeleteCompany, handleRenameCompany }) => {
    const [companyTab, setCompanyTab] = useState('chat');
                         ...
                         <div className="h-full flex flex-col">
                             ...
                             <div className="space-y-3 flex-grow overflow-y-auto custom-scrollbar pr-2">
                                 {membersInCompany.map(member => (
                                     <div key={member.id} className="bg-gray-800/30 rounded-lg p-3 flex items-center gap-3">
                                         <img src={member.profilePictureUrl} className="w-12 h-12 rounded-full border-2 border-gray-600" onError={(e) => { e.target.onerror = null; e.target.src='https://placehold.co/48x48/090979/ffffff?text=U'; }} alt={member.name}/>
                                         <div className="flex-grow">
                                             <p className="font-bold text-white text-sm">{member.name}</p>
                                             <p className="text-xs text-gray-400">{levelNames[(member.level || 1)-1] || 'Rookie'}</p>
                                             {member.id === userCompany.ownerId && <span className="text-xs font-bold text-yellow-400">👑 ASOSCHI</span>}
                                         </div>
                                         <div className="text-right">
                                             <p className="font-semibold text-lg text-yellow-400">{formatNumberShort(member.wealth || member.dubaiCoin || 0)}</p>
                                         </div>
                                     </div>
                                 ))}
                             </div>
