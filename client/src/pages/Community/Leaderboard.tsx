...
const Leaderboard = ({ leaderboard, companies, user, donators, openSimplePaymentModal, levelNames }) => {
    ...
    const personalLeaderboard = [...safeLeaderboard, {rank: '...', name: user.username, wealth: user.dubaiCoin, level: user.level || 1, profilePictureUrl: user.profilePictureUrl}]
        .sort((a, b) => (b.wealth || 0) - (a.wealth || 0)) // Balans bo'yicha tartiblash - eng yuqori birinchi
        .map((p, i) => ({...p, rank: i+1}));
        ...
        <div className="h-full flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar pr-2">
                ...
                {tab === 'donators' && (
                    <>
                        <button 
                            onClick={() => openSimplePaymentModal({
                                name: 'Homiylik',
                                usdPrice: 5.0
                            }, () => {
                                // Simple notification after payment
                                alert('Homiylik uchun rahmat!');
                            })}
                            className="w-full mb-4 bg-yellow-500 text-black font-bold py-2 px-3 rounded-lg"
                        >
                            Homiy bo'lish ($5)
                        </button>
