...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/user', async (req, res) => {
    try {
      ...
      const referralCode = `DC${userData.telegramId.slice(-6)}`;
      ...
      const newUserData = {
        ...
        profileImage: null,
        referralCode,
        referredBy: userData.referredBy ? parseInt(userData.referredBy) : null,
        premiumStatus: false,
        premiumUntil: null,
        achievements: [],
        dailyRewards: {
      ...
      if (userData.referredBy) {
        try {
          const referrerId = parseInt(userData.referredBy);
