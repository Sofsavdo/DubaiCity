...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/health", (req, res) => {
    res.json({
      success: true,
      message: "Dubai City Bot API is running",
  ...
  app.post("/api/admin/auth/login", async (req, res) => {
    try {
      ...
      // Simple admin authentication (production da JWT token ishlatish kerak)
      if (username === "admin" && password === "dubai2024") {
        const adminUser = {
          id: 1,
          username: "admin",
          isAdmin: true,
  ...
  app.get("/api/admin/stats", async (req, res) => {
    try {
      ...
      // If no stats available, return demo data
      const stats = {
        users: userStats || {
          total: 0,
          active: 0,
